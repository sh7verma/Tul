    ...
    public void onBindViewHolder(final ViewHolder holder, int position) {
        ...
        if (holder.Holderid == 0) {
            ...
            holder.tvTulPrice.setText(mData.get(position).getCurrency()+ " "
                    + String.format("%.2f", Double.parseDouble(mData.get(position).getTotal_amount())));
            ...
            try {
                if (mData.get(position).getBorrower_status() == 0) {
                    ...
                    holder.tvtulRecievedDate.setText(Constants.convertDate(mData.get(position).getBorrower_received_at()));
                    ...
                    holder.tvTulReturnedDate.setText(Constants.convertDate(mData.get(position).getBorrower_returned_at()));
                }
            } catch (Exception e) {
                Log.e("Exce  = ", e + "");
            }
            ...
            holder.tvCancel.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    double percentage = Double.parseDouble(mData.get(finalPosition).getCancel_percentage());
                    double price = Double.parseDouble(mData.get(finalPosition).getBase_price());
                    double cancellationCharges = (price * percentage) / 100;
                    final String mCancelationCharges = mData.get(finalPosition).getCurrency()+ " " + String.format("%.2f", cancellationCharges);
                    ((ActiveBookingActivity) mContext).cancelTulBooking(String.valueOf(mData.get(finalPosition).getTool_id()),
                            String.valueOf(mData.get(finalPosition).getBooking_id()), finalPosition, mCancelationCharges);
                }
            });
            ...
            holder.rlTulReceived.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    if (mData.get(finalPosition).getLender_status() == 1 && mData.get(finalPosition).getBorrower_status() == 0) {
                        ...
                        /// show toast message.
                        Toast.makeText(mContext, R.string.please_contact_lender, Toast.LENGTH_SHORT).show();
                    }
                }
            });
    ...
    public class ViewHolder extends RecyclerView.ViewHolder {
        ...
        public ViewHolder(View itemView, int viewType) {
            ...
            if (viewType == TYPE_HEADER) {
                ...
                llMyBooking.setBackground(ContextCompat.getDrawable(mContext, R.drawable.black_stroke_default));
