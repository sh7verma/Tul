...
public class SaleVerifyPhoneNumber extends Activity implements View.OnClickListener {
...
// ---retrieve the SMS message received---
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mContext = SaleVerifyPhoneNumber.this;
...
//        llMain.setLayoutParams(mainParms);
    ...
    private void setData() {
            txtVerifyMsg.setText(getResources().getString(R.string.verfication_hint_msg) + " "
...
//                Toast.makeText(mContext, "Work in Progress", Toast.LENGTH_SHORT).show();
    ...
    private void hitResendAPI() {
        ...
        Call<SignupModel> call = RetrofitClient.getInstance().callSellerResend(util.getString("access_token", ""),
        ...
        call.enqueue(new Callback<SignupModel>() {
            ...
            public void onResponse(Call<SignupModel> call, Response<SignupModel> response) {
                if (response.body().getResponse() != null) {
                    ...
                    startTimer();
                    disableButtons();
                    Toast.makeText(mContext, R.string.otp_successfully_set, Toast.LENGTH_SHORT).show();
                } else {
    ...
    private void hitCallAPI() {
        ...
        Call<SignupModel> call = RetrofitClient.getInstance().callSellerUser(util.getString("access_token", ""),
                getIntent().getStringExtra("code"),
                getIntent().getStringExtra("phone"));
        ...
        call.enqueue(new Callback<SignupModel>() {
            ...
            public void onResponse(Call<SignupModel> call, Response<SignupModel> response) {
                CustomLoadingDialog.getLoader().dismissLoader();
                if (response.body().getResponse() != null) {
                    Toast.makeText(mContext, R.string.call_initiated_successfully, Toast.LENGTH_SHORT).show();
                    util.setInt("otp", response.body().getResponse().getOtp());
                } else {
                    if (response.body().error.getCode().equals(getResources().getString(R.string.invalid_access_token))) {
                        Constants.moveToSplash(mContext, util);
    ...
    private void hitAPI() {
        CustomLoadingDialog.getLoader().showLoader(mContext);
        Call<SignupModel> call = RetrofitClient.getInstance().userSellerVerify(util.getString("access_token", ""),
                mStringBuilder.toString(), getIntent().getStringExtra("code"), getIntent().getStringExtra("phone"));
        call.enqueue(new Callback<SignupModel>() {
            @Override
            public void onResponse(Call<SignupModel> call, Response<SignupModel> response) {
                if (response.body().getResponse() != null) {
