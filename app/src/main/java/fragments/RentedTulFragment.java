...
public class RentedTulFragment extends Fragment implements View.OnClickListener {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        ButterKnife.bind(RentedTulFragment.this, itemView);
    ...
    public void setLocalData() {
        mListArray.clear();
        mListArray.addAll(db.getAllHistoryRentedTuls());
...
//                    }
    ...
    private void addToLocal(List<BookTulModel.ResponseBean> response) {
        for (int i = 0; i < response.size(); i++) {
            db.addHistoryRentedTul(response.get(i));
            for (AttachmentModel attachmentItem : response.get(i).getAttachment()) {
                db.addHistoryRentedAttachment(attachmentItem);
            }
        }
    }
