...
import static com.prolificinteractive.materialcalendarview.MaterialCalendarView.mCalenderDisabledDates;
...
//            isDayEnabled(day)
            dayView.setupSelection(
                    showOtherDates, day.isInRange(minDate, maxDate), true);
        }
        postInvalidate();
    }
        ...
        for (DayView dayView : dayViews) {
            ...
            for (DecoratorResult result : decoratorResults) {
                if (result.decorator.shouldDecorate(dayView.getDate())) {
                    mCalenderDisabledDates.add(dayView.getDate());
